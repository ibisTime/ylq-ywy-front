        ...
        <ul>
          <li class="line-item cd-flexbox cd-align-center" v-for="item in list" @click="toCheck(item.code)">
            ...
            <div class="info over-hide par0">
              ...
              <div>
                <p class="note over-hide" v-if="item.status === '1'">待处理</p>
                <p class="note over-hide" v-if="item.status === '2'">已否决</p>
                <p class="note over-hide" v-if="item.status === '3'">已通过</p>
              </div>
            </div>
          </li>
        </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toCheck(code) {
        this.$router.push('/home/orders/check?code=' + code);
      },
      nothing() {},
      toggleStatus(index) {
        this.tabIndex = index;
